...
function UserImage() {
    ...
    async function submit(data) {
        setPersonalInfo({ ...personalInfo, "name": data.name })
        await setEdit(false)
    }
    ...
    return (
        <div className={CSS.imageContainer}>
            ...
            {(edit)?
            <input type="text" className={`${CSS.input} ${CSS.name}`} defaultValue={personalInfo.name} {...register("name", {
                required: "fill this input"
            })} style={(errors.name) ? { borderColor: "red" } : { borderColor: "black" }} />
            :
            <h2 className={CSS.name}>{personalInfo.name}</h2>
            }
        </div>
    )
}
